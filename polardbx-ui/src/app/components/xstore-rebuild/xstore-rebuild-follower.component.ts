...
interface RebuildFollowerRequest {
  name: string;
  xStoreName: string;
...
@Component({
  selector: 'app-xstore-rebuild-follower',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  template: `
    <div class="follower-rebuild-page">
      <div class="layout-container">
        <div class="page-header">
          <div class="header-content">
            <h1 class="page-title">
              <i nz-icon nzType="sync" class="page-icon"></i>
              备库重搭 · 重搭 Follower 节点
            </h1>
            <p class="page-description">选择目标 XStore 的 Follower Pod 并触发重搭流程</p>
          </div>
        </div>
            ...
            <div class="card-content">
              <form nz-form nzLayout="vertical" (ngSubmit)="submit()">
                ...
                <nz-alert
                  ...
                  nzShowIcon
                  class="helper-alert"
                  nzMessage="候选 Pod 会实时查询，优先展示运行中的非 Leader 节点">
                </nz-alert>

                <nz-form-item class="pod-select-item">
                  <nz-form-label nzRequired>目标 Pod</nz-form-label>
                  <nz-form-control>
                    <div class="pod-select-row">
                      <nz-select
                        ...
                        name="targetPod"
                        [ngModelOptions]="{ standalone: true }"
                        nzPlaceHolder="选择运行中的 Follower Pod"
                        nzShowSearch
                ...
                <nz-form-item>
                  <nz-form-label>源 XStore (可选)</nz-form-label>
                  <nz-form-control>
                    <input
                      nz-input
                      id="follower-from-input"
                      name="fromXStore"
                      [(ngModel)]="fromXStore"
                      [ngModelOptions]="{ standalone: true }"
                      placeholder="不指定则自动选择" />
                    <div class="form-hint">可指定同步数据来源的 XStore，未填写时由系统自动决策。</div>
                  </nz-form-control>
                </nz-form-item>
                ...
                <nz-form-item>
                  ...
                  <nz-form-control>
                    <input
                      ...
                      [ngModelOptions]="{ standalone: true }"
                      placeholder="留空自动生成" />
                    <div class="form-hint">不填写将生成带有 Follower 标识的任务名称。</div>
                  </nz-form-control>
                </nz-form-item>
...
export class XStoreRebuildFollowerComponent {
  namespace = 'default';
  xstore = '';
  targetPod = '';
  fromXStore = '';
  ...
  private genName(base: string): string {
    ...
    const raw = `${base}-follower-${ts}-${rand}`.toLowerCase().replace(/[^a-z0-9-]/g,'-');
  ...
  submit(): void {
    ...
    const targetPod = (this.targetPod || '').trim();

    if (!xstore || !targetPod) {
      this.message.warning('请完善必填项：目标 XStore 和目标 Pod');
      return;
    }

    const req: RebuildFollowerRequest = {
      name: (this.name || '').trim() || this.genName(xstore),
      xStoreName: xstore,
      targetPodName: targetPod || undefined,
      fromXStore: (this.fromXStore || '').trim() || undefined
    };
    this.submitting = true;
    this.api.rebuildFollower(namespace, req).subscribe({
      next: () => {
        this.message.success('已提交 Follower 重搭任务');
    ...
    this.api.getXStorePods(namespace, xstore).subscribe({
      next: (pods: XStorePod[] | null | undefined) => {
        ...
        this.filteredPods = this.pods.filter(p => {
          ...
          const role = p.metadata?.labels?.['xstore/role'];
          return phase === 'Running' && role !== 'leader';
        });
        if (!this.filteredPods.length) {
          this.filteredPods = this.pods.filter(p => p.status?.phase === 'Running');
        }
        if (!this.filteredPods.length) {
          this.message.info('未找到符合条件的运行中 Follower Pod');
        } else {
