...
func TestBackupOverview_EvaluateConnectivity(t *testing.T) {
	...
	r.GET("/api/v1/backups/overview", api_backup.GetBackupOverview)
	...
	assert.Equal(t, "unknown", kpi2["storageConnectivity"])
	...
	r2.GET("/api/v1/backups/overview", api_backup.GetBackupOverview)
	...
	assert.Equal(t, "connected", kpi3["storageConnectivity"])
}
