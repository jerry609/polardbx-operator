# 快速开始指南 - All-in-One 部署

本指南帮助新用户快速部署 PolarDB-X 管理平台（前后端统一镜像）到 Kubernetes 集群。
...
```bash
# 将 kubeconfig 编码为 base64（用于 Secret）
KUBECONFIG_B64=$(cat ~/.kube/config | base64 -w 0)
# macOS 使用：KUBECONFIG_B64=$(cat ~/.kube/config | base64)
```
...
helm install polardbx-ui tools/helm-example \
  ...
  --set backend.secrets.kubeconfig="$KUBECONFIG_B64" \
...
# 查看日志
kubectl logs -n polardbx-system -l app.kubernetes.io/component=backend
...
```bash
# 使用 Helm
helm uninstall polardbx-ui -n polardbx-system
...
- 配置持久化存储（如需要）
- 配置监控和告警
- 查看 [README-ALL-IN-ONE.md](./README-ALL-IN-ONE.md) 了解更多配置选项
